class StreamInfoParser {
  ...
  int parseContentLength() => null; // Regex.Match(ParseUrl(), @"clen[/=](\d+)").Groups[1].Value.ParseLong();

  int parseBitrate() => null; // (long) _root.Attribute("bandwidth");

  bool parseIsAudioOnly() => null; // _root.Element("AudioChannelConfiguration") != null;
